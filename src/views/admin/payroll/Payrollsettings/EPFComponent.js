...
const contributionOptions = [
  ...
  // { name: 'prorate_restricted_pf_wage', label: 'Pro-rate Restricted PF Wage' }
];
...
function EpfComponent({ handleNext }) {
  ...
  const handleCheckboxChange = (event, fieldName) => {
    const updatedRates = event.target.checked
      ? [...values.employer_contribution_rate, fieldName] // Add the fieldName if checked
      : values.employer_contribution_rate.filter((rate) => rate !== fieldName); // Remove the fieldName if unchecked

    setFieldValue('employer_contribution_rate', updatedRates); // Update Formik state
  };
                ...
                {contributionOptions.map((field) => (
                  <Grid2 xs={12} sm={6} key={field.name}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={values.employer_contribution_rate.includes(field.name)}
